(function(global){
  ...
  async function loadPrivatePositions(){
    ...
    try{
      ...
      tbody.innerHTML=active.length?active.map(x=>{const cached=JSON.parse(sessionStorage.getItem(`optionQuote:${x.id}`)||'null');return renderPositionRow(x,cached?.quote,cached?`缓存报价 · ${new Date(cached.at).toLocaleTimeString()}`:'点击“刷新”读取行情')}).join(''):'<tr><td colspan="14" style="text-align:center;color:var(--muted)">当前没有开放或待结算的期权持仓</td></tr>';
      renderRiskSummary();
  ...
  function renderRiskTodo(){
    ...
    state.positions.forEach(p=>{const cached=JSON.parse(sessionStorage.getItem(`optionQuote:${p.id}`)||'null'),risk=positionRisk(p,cached?.quote);if(risk.level==='l2'||risk.level==='l3')items.push({level:risk.level,text:`${p.symbol} ${p.expiry}：${risk.label.replace(/^L[23]\s*/, '')}`})});
  ...
  function renderPositionRow(x,quote=null,message='点击“刷新”读取行情'){
    ...
    return `<tr id="opt-row-${x.id}"><td>${x.symbol} <span class="badge neutral">${x.side} ${x.opt_type}</span></td><td>$${Number(x.strike).toFixed(2)}</td><td>${x.expiry} (${dte}d)</td><td>$${Number(x.cost).toFixed(2)} / 每股</td><td>${m&&Number.isFinite(m.mark)?money(m.mark):'—'}</td><td class="${m?pnlClass:''}">${m&&Number.isFinite(m.pnl)?money(m.pnl):message}</td><td class="${m?pnlClass:''}">${m&&Number.isFinite(m.pnlPct)?pct(m.pnlPct):'—'}</td><td>$${Number(m?.breakeven??(String(x.opt_type).toLowerCase()==='put'?Number(x.strike)-Number(x.cost):Number(x.strike)+Number(x.cost))).toFixed(2)}</td><td>${Number.isFinite(underlying)?money(underlying):'—'}</td><td>${Number.isFinite(distance)?pct(distance):'—'}</td><td>${rocHtml}</td><td><span class="risk-chip ${risk.level}${riskEscalationClass(x.id,risk.level)}" title="${eventText}">${risk.label}</span></td><td>${Number.isFinite(iv)?pct(iv):'—'} / ${Number.isFinite(delta)?delta.toFixed(3):'—'}</td><td><button onclick="OptionV2.refreshPosition('${x.id}')" title="刷新报价">↻</button> <button onclick="OptionV2.openPositionScenario('${x.id}')" title="按真实成本推演">推演</button>${rocButton} <button onclick="deleteOptionPosition(${x.id})" title="删除">🗑️</button></td></tr>`;
  }
  ...
  global.OptionV2={bsPrice,evaluate,normalizeColumnar,loadPrivatePositions,refreshQuote,refreshPosition,openPositionScenario,completeRocFields,occSymbol,positionMetrics,annualizedRoc,positionRisk,setMarketEvents};
