...
# 930955（中证红利低波100指数）由中证指数公司编制，非沪深主板股票代码，
# 用 sh 前缀能否直接查到需要实测；查不到时的 fallback 是改用东方财富接口。
CN_HK_SYMBOLS = {
    "sh930955": {"name": "中证红利低波100指数", "kind": "index"},
    "sz159307": {"name": "红利低波100ETF(场内)", "kind": "etf"},
...
def fetch_fund_estimate(fund_code):
    ...
    """
    raw = _http_get_text(FUND_EST_URL.format(code=fund_code))
    # 格式形如: jsonpgz({...});
    json_str = raw[raw.find("{"):raw.rfind("}") + 1]
    try:
        data = json.loads(json_str)
    except json.JSONDecodeError:
        return {"error": "解析失败，接口可能改版了"}
    return {
        "name": data.get("name"),
