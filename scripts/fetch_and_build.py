...
def render_html(data):
    ...
    for sym,tp in trend_data.items():
        ...
        trend_cards.append(f'''<article class="trend-card trend-card-v45" data-tone="{tone}"><div class="trend-card-top"><div><span>{html.escape(STOCK_META.get(sym,{}).get("name",sym))}</span><h3>{sym}</h3></div><div class="trend-score-xl {tone}">{tp.get("score",0):+.0f}</div></div><div class="trend-state-row"><span class="trend-state {tone}">{html.escape(state_name)}</span><span class="trend-action-chip {action_tone}" title="{html.escape(action_desc)}">{html.escape(action_label)}</span></div><div class="trend-card-grid"><div class="trend-metric"><span>周线</span><b>{html.escape(tp.get("weekly","-"))}</b></div><div class="trend-metric"><span>5日斜率</span><b>{tp.get("slope5",0):+.1f}</b></div><div class="trend-metric"><span>Supertrend</span><b>{html.escape(tp.get("supertrend","-"))}</b></div><div class="trend-metric"><span>ADX</span><b>{fmt_num(tp.get("adx"),1)}</b></div><div class="trend-metric"><span>+DI / -DI</span><b>{fmt_num(tp.get("plus_di"),1)} / {fmt_num(tp.get("minus_di"),1)}</b></div><div class="trend-metric"><span>结构</span><b>{html.escape(tp.get("structure","-"))}</b></div></div><div class="trend-action-explain"><b>动作解释</b><span>{html.escape(action_desc)}</span></div><p class="trend-analysis"><b>分析：</b>{html.escape(tp.get("analysis",""))}</p><div class="trend-integrity integrity-{isty}"><span>数据状态</span><strong>{html.escape(integrity.get("label","待校验"))}</strong><button class="info-tip" type="button" aria-label="数据状态说明" data-tooltip="{html.escape(integrity_desc)}">ⓘ</button><small>{html.escape(str(integrity.get("as_of","-")))}</small></div></article>''')
...
</style><link href="assets/dashboard-v2.2.css?v={ASSET_VERSION}" rel="stylesheet"><link href="assets/design-v4.5.css?v={ASSET_VERSION}" rel="stylesheet"><link href="assets/design-v4.5.1.css?v={ASSET_VERSION}" rel="stylesheet"></head><body class="auth-pending" data-app-version="{APP_VERSION}"><div class="app">
